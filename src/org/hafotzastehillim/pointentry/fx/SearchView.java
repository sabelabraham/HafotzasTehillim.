...
import org.hafotzastehillim.pointentry.fx.cell.EntryCell;
...
public class SearchView extends VBox {
	...
	public SearchView() {
		...
		query.setOnAction(evt -> {
			...
			// Hidden feature! 
			if (query.getText().endsWith("Z")) { // decipher date if possible
				try {
					Instant time = Instant.parse(query.getText());
					ZonedDateTime zoned = time.atZone(ZoneId.systemDefault());

					DialogUtils.createAlert(AlertType.INFORMATION, "Date and Time", DAY_FORMATTER.format(zoned),
