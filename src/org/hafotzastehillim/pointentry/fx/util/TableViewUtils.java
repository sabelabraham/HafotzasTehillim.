...
import org.hafotzastehillim.pointentry.fx.GiftResultsView;
...
public class TableViewUtils {
	...
	public static TableView<Entry> getTable(ObservableList<Entry> items) {
		...
		Selectable.bindCheckbox(items, columnBox);

		select.setGraphic(columnBox);
		select.setCellFactory(col -> new CheckBoxTableCell<>());
		...
		table.getColumns()
				.addAll(Arrays.asList(number, select, account, id, memberSince, gender, firstName, lastName,
						addressNumber, addressName, apt, city, state, zip, cityYiddish, age, school, phone, cellPhone,
						fatherName, lastNameYiddish, firstNameYiddish, total));
	...
	public static TableView<FamilyGrouping> getFamilyTable(ObservableList<FamilyGrouping> items) {
		...
		table.getColumns().addAll(Arrays.asList(number, select, account, lastName, addressNumber, addressName, apt,
				city, state, zip, cityYiddish, phone, cellPhone, fatherName, lastNameYiddish));
