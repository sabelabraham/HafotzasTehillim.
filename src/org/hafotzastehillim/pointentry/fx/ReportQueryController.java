...
import org.hafotzastehillim.pointentry.fx.util.RangeParser;
...
public class ReportQueryController {
	...
	public void initialize() {
		...
		DialogUtils.dialogButtonStyling(run);
	...
	public void run(ActionEvent event) {
		...
		if (campaignSelectToggle.isSelected()) {
			try {
				...
				DialogUtils.createAlert(AlertType.ERROR, "Error", "Error", "Error in Campaign select:\n" + e.getMessage());
				return;
			}
		}
		...
		Model.getInstance().getSpreadsheet().searchService().setOnSucceeded(evt -> {
			...
			Platform.runLater(() -> DialogUtils.createDialog(report, "Reports", ButtonType.OK));
		});
